...
from rich.progress import Progress,track, TextColumn, BarColumn, TimeElapsedColumn, TimeRemainingColumn,SpinnerColumn
...
class BaseRunner:
    ...
    def train(self):
        ...
        with Progress( SpinnerColumn('smiley'),
            TextColumn("[yellow][Training]{task.description}"),
              BarColumn(),
            ...
            step_tqdm = progress.add_task(description="step progress", total=self.cfg.max_steps)
            for global_step in range(self.cfg.max_steps):
                ...
                progress.advance(step_tqdm, advance=1)
    ...
    #     torch.save(self.model.state_dict(), os.path.join(self.cfg.output_dir, 'checkpoint/pytorch_model.bin'))
    #     config_path = os.path.join(self.cfg.output_dir, 'checkpoint/config.json')
    #     with open(config_path, 'w') as config_file:
    #         json.dump(self.cfg.__dict__, config_file)
