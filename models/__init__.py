...
def build_model(args):
    ...
    config.num_proto_per_type = args.num_proto_per_type
    config.max_iter = args.max_iter
    config.role2id = json.load(open(args.role2id_file,'r'))
    if os.path.exists(args.hpnfile):      
        protos = torch.from_numpy(np.load(args.hpnfile)).float()
    else:
        protos = torch.randn(
            (len(config.role2id), args.num_proto_per_type, config.hidden_size),
            requires_grad=False
        )
    config.role2id = json.load(open(args.role2id_file,'r'))
    config.pos_loss_weight = args.pos_loss_weight
    tokenizer = tokenizer_class.from_pretrained(args.model_name_or_path)
    if args.inference_only:
        model = model_class.from_pretrained(args.inference_model_path, config=config,
                                            num_prompt_pos=args.num_prompt_pos, num_event_embed=args.num_event_embed,protos=protos )
    else:
